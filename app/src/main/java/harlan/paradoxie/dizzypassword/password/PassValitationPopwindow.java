...
public class PassValitationPopwindow extends PopupWindow implements
    ...
    private final static int NUMBER_COUNT = 8;
...
//		mMenuView = LayoutInflater.from(mContext).inflate(R.layout.pop_view, null);
        ...
        mNumber7TextView = (TextView) mMenuView.findViewById(R.id.number_7_textView);
        mNumber8TextView = (TextView) mMenuView.findViewById(R.id.number_8_textView);
        ...
        mNumberViewList.add(mNumber7TextView);
        mNumberViewList.add(mNumber8TextView);
    ...
    private void validation(String code) {
        try {
            if (aCache.getAsString(Constant.PD).equals(EntryptionHelper.decrypt(code, Constant.PD)) && type == 0) {
                mCallback.onSuccess();
               // dismiss();
            } else if (aCache.getAsString(Constant.PD).equals(EntryptionHelper.decrypt(code, Constant.PD)) && type == 1) {
                mCallback.onSuccess();
                //dismiss();
            } else {
                clearnNumber();
//			Toast.makeText(mContext, "验证失败,请重新验证", Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText(mContext, "密码解密出错", Toast.LENGTH_LONG).show();
        }
